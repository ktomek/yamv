...
internal class FeatureFilter(private val resolver: Resolver) {
    ...
    fun findAll(): List<KSDeclaration> {
        val classes = AutoFeatureDiscovery
            .findAnnotatedClasses(resolver)
            .filter { it.isKnownFeatureClass() }
        val properties = AutoFeatureDiscovery
            .findAnnotatedProperties(resolver)
            .filter { it.isKnownFeatureProperty() }
    ...
    fun forState(
        ...
        allFeatures: List<KSDeclaration>,
    ): List<KSDeclaration> {
        val stateName = stateClass.toClassName().simpleName
        return allFeatures.filter { it.validate() && it.matchesState(stateName) }
    }

    private fun KSClassDeclaration.isKnownFeatureClass(): Boolean = superTypes
        .any { it.resolve().declaration.qualifiedName?.asString() in KNOWN_CLASS_FEATURE_FQNS }
    ...
    private fun KSTypeReference.stateTypeNameMatches(stateName: String): Boolean = resolve()
        .arguments
        .firstOrNull()
        ?.type
        ?.resolve()
    ...
    companion object {
        private val KNOWN_CLASS_FEATURE_FQNS = setOf(
            KoinFeatureFqns.FLOW_FEATURE,
            KoinFeatureFqns.FUNCTION_TYPED_FEATURE,
        )
